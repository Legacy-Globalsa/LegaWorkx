...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: any }) => {
  ...
  useEffect(() => {
    ...
    class CarLights {
      webgl: App; options: any; colors: any; speed: any; fade: any;
      ...
      init() {
        ...
        const aOffset: number[] = []; const aMetrics: number[] = []; const aColor: number[] = [];
        let colors = this.colors;
        if (Array.isArray(colors)) colors = colors.map((c: number) => new THREE.Color(c));
      ...
      update(time: number) { if (this.mesh) (this.mesh.material as THREE.ShaderMaterial).uniforms.uTime.value = time; }
    }
    ...
    class LightsSticks {
      ...
      init() {
        ...
        const aOffset: number[] = []; const aColor: number[] = []; const aMetrics: number[] = [];
    ...
    options.distortion = distortions[options.distortion as string] || distortions['turbulentDistortion'];
